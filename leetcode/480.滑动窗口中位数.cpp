 ...
 public:
    class hash_heap {
     private:
        ...
        int get_value(int index) { return _data[index]; }
        ...
        void heap_down(int i = 0) {
            ...
            while (2 * i + 1 < sz) {
                ...
                int ma = mm + 1;
                if (ma < sz &&
                    _cmp(get_value(_indexes[ma]), get_value(_indexes[mm]))) {
                    mm = ma;
                }
     ...
     public:
        explicit hash_heap(const vector<int> &data,
                           function<bool(int, int)> cmp = less_equal<int>{})
        ...
        void pop(int inner_index = 0) {
            ...
            if (inner_index == 0) {
                ...
                if (_cmp(get_value(_indexes[inner_index]),
                         get_value(_indexes[p]))) {
                    heap_up(inner_index);
                } else {
                    heap_down(inner_index);
                }
        ...
        bool check(int i = 0) {
            ...
            if (ma < sz) {
                if (!_cmp(get_value(_indexes[i]), get_value(_indexes[mm])) ||
                    !_cmp(get_value(_indexes[i]), get_value(_indexes[ma]))) {
                    return false;
                }
                return check(mm) && check(ma);
            }
            return _cmp(get_value(_indexes[i]), get_value(_indexes[mm])) &&
                   check(mm);
        }
        ...
        void print() {
            for (auto &kv : _index_map) {
                cout << kv.first << "\t" << kv.second << "\t"
                     << get_value(_indexes[kv.first]) << "\t"
                     << _data[kv.second] << endl;
            }
        }
    };
    ...
    class medianWindows {
     private:
        ...
        double median() {
            if (sz % 2 == 0) {
                return (double)((long long)data[mid] + data[max_heap.top()]) /
                       2.0;
            }
            return data[mid];
        }
     ...
     public:
        medianWindows(const vector<int> &data, int k)
            : data(data),
              min_heap(data),
              max_heap(data, greater_equal<int>{}),
              k(k),
              mid(-1),
              cur(0),
              sz(0) {
    ...
    class violateWindows {
     ...
     public:
        violateWindows(const vector<int> &data, int k)
            : data(data), cur(0), k(k) {}
