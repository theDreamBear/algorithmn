...
bool check_G_mod_k(ll k) {
    // k = 1 的特殊情况
    if (k == 1) return true;

    // 计算 Graham's Number mod k
    ll now = 2;  // G[0] = 4, log_3(4) ≈ 1.262, 所以从2开始
    for (int i = 0; i < 64; i++) {
        now = qpow(3, now, k);
        // 如果中间结果为0，说明G能被k整除
...
void solveA() {
    def(ll, k);
    cout << (check_G_mod_k(k) ? "yes" : "no") << endl;
}
...
void solveB() {
    ...
    auto helper = cache<Mint(int)>(memo);
    vl ans(n);
    rep(i, 1, n + 1) {
        ans[i - 1] = helper(i).v;
    }
...
void solveC() {
    ...
    //ll mod = 998244353;
//    ll fz = quickMi(quickMi(2, n - 1, mod) + 1, k, mod);
//    ll fm = quickMi(2, n * k, mod);
//    cout << (fz * quickMi(fm, mod - 2, mod) % mod) << endl;

    cout << mod_pow((mod_pow(2, n - 1) + 1), k) * mod_inv(mod_pow(2, n * k)) << endl;
}
...
void shell_sort(vector<int>& data) {
    ...
    for (int gap = max_gap; gap >= 1; gap  = (gap - 1) / 3) {
        // 插入排序
        for (int i = gap; i < n; i++) {
            int j = i - gap;
            int v = data[i];
    ...
    cout << is_sorted(data.begin(), data.end());
}
...
void solveE() {
    ...
    struct Node {
        ...
        Mint getv() {
            return v - (int)bs.count();
        }
    };
    auto memo = [&](auto&& dfs, int u)->Node {
        Node node;
        node.bs.set(u);
        for (auto v : g[u]) {
            node += dfs(v);
    ...
    auto helper = cache<Node(int)>(memo);
    vl ans(n);
    rep(i, 1, n + 1) {
        ans[i - 1] = helper(i).getv().v;
    }
    ps(ans);
}
