...
public:
    ...
    int maxProfit1(int k, vector<int>& prices) {
        // 寻找所有的递增区间. 但是答案并不一定是最高峰
        // 当seg.size() > k 需要进行区间合并
        //
        vi seg;
        int low = prices[0];
        irep(prices.size()) {
            while (i + 1 < prices.size() and prices[i] > prices[i + 1]) i++;
            if (i + 1 >= prices.size()) break;
            int start = i;
            while (i + 1 < prices.size() and prices[i] <= prices[i + 1]) i++;
            seg.emplace_back(prices[i] - prices[start]);
        }
        int ans = 0;
        if (seg.size() >= k) {
            ranges::sort(seg);
        }
        irep(min(k, (int)seg.size())) {
            ans += seg[i];
        }
        return ans;
    }
    ...
    int maxProfit(int k, vector<int>& prices) {
        // 后往前
        auto handler = [&](int i, int j, int hold)->pair<bool, int> {
            if (j < 0) {
                return {true, INT_MIN / 2};
            }
            if (i < 0) {
                return {true, hold == 0 ? 0 : INT_MIN / 2};
            }
            return {false, 0};
        };

        auto memo = [&](auto&& dfs, int i, int j, int hold)->int {
            if (hold) {
                return max(dfs(i - 1, j - 1, 0) - prices[i], dfs(i - 1, j, 1));
            }
            return max(dfs(i - 1, j, 1) + prices[i], dfs(i - 1, j, 0));
        };
        //return cache<int(int, int, int)>(memo, handler)((int)prices.size() - 1, k, 0);
        return cache_vec3<int(int, int, int)>(memo, (int)prices.size(), k + 1, 2, INT_MIN / 2, handler)((int)prices.size() - 1, k, 0);
        //return cache_nvec<3, int(int, int, int)>(memo, {(int)prices.size(), k + 1, 2}, INT_MIN / 2, handler)((int)prices.size() - 1, k, 0);
    }
};
...
void test() {
    ...
    ofstream inn(inputFile);
        ...
        for (int i = 0; i < times; i++) {
            auto v = generateRandomArray(sz, low, high);
            inn << sz << endl;
            inn << v << endl;
            // 对比输出到文件
            brute(v);
