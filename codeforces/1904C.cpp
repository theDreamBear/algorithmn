...
void solveA() {
    ...
    if (k == 2) {
        for (auto [i, v] : range(a)) {
            int j = i - 1;
            for (int t = 0; t < i; t++) {
                ll d = v - a[t];
                while (j >= 0 and d < a[j]) j--;
                // j + 1 是 > d 的最小下标
                ans = min(ans, a[j + 1] - d);
                if (j >= 0) {
                    ans = min(ans, d - a[j]);
...
<meta property="og:title" content="3185. 构成整天的下标对数目 II - 力扣（LeetCode）">
<meta property="og:description" content="3185. 构成整天的下标对数目 II - 给你一个整数数组 hours，表示以 小时 为单位的时间，返回一个整数，表示满足 i < j 且 hours[i] + hours[j] 构成 整天 的下标对&nbsp;i, j 的数目。
