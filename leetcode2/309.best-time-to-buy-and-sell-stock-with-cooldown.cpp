...
class Solution {
  public:
    int maxProfi1(vector<int>& prices) {
        // i, h
        ...
        auto memo = [&](auto&& dfs, int i, int h) {
            if (h) {
                return max(dfs(i - 1, 1), dfs(i - 2, 0) - prices[i]);
            }
            return max(dfs(i - 1, 0), dfs(i - 1, 1) + prices[i]);
        };
        // return cache<int(int, int)>(memo, handler)((int)prices.size() - 1, 0);
        return cache_nvec<2, int(int, int)>(memo, {(int)prices.size(), 2}, INT_MIN / 2, handler)((int)prices.size() - 1, 0);
    }
    ...
    int maxProfit2(vector<int>& prices) {
        ...
        for (int i = 0; i < n; i++) {
            dp[i][1] = max(get_v(i - 1, 1), get_v(i - 2, 0) - prices[i]);
            dp[i][0] = max(get_v(i - 1, 0), get_v(i - 1, 1) + prices[i]);
        }
        return dp[n - 1][0];
    }
    ...
    int maxProfit(vector<int>& prices) {
        ...
        int n = prices.size();
        int dp[2][3];
        fill_array(&dp[0][0], INT_MIN / 2, 2, 3);
        int ft = 0, st = 1;
        auto get_v = [&](int i, int h) {
            if (auto res = handler(i, h); res.first) {
                return res.second;
        ...
        for (int i = 0; i < n; i++) {
            dp[st][0] = max(get_v(i - 1, 0), get_v(i - 1, 2));
            dp[st][1] = max(get_v(i - 1, 1), get_v(i - 1, 0) - prices[i]);
            dp[st][2] = get_v(i - 1, 1) + prices[i];
            swap(ft, st);
        }
        return max({dp[ft][0], dp[ft][1], dp[ft][2]});
    }
};
// @lc code=end
