...
class Solution {
  public:
    int maxProfit_tle(vector<int>& prices, int fee) {
        ...
        int n = prices.size();
        return cache_nvec<2, int(int, int)>(memo, {n, 2}, INT_MAX, handler)(n - 1, 0);
    }
    ...
    int maxProfit1(vector<int>& prices, int fee) {
        ...
        for (int i = 0; i < n; i++) {
            dp[i][0] = max(get_v(i - 1, 0), get_v(i - 1, 1) + prices[i] - fee);
            dp[i][1] = max(get_v(i - 1, 1), get_v(i - 1, 0) - prices[i]);
        }
        return dp[n - 1][0];
    }
    ...
    int maxProfit(vector<int>& prices, int fee) {
        ...
        int dp[2][2];
        int ft = 0, st = 1;
        fill_array(&dp[0][0], INT_MIN / 2, 2, 2);
        auto get_v = [&](int i, int h) {
            if (auto res = handler(i, h); res.first) {
                return res.second;
            }
        ...
        for (int i = 0; i < n; i++) {
            dp[st][0] = max(get_v(i - 1, 0), get_v(i - 1, 1) + prices[i] - fee);
