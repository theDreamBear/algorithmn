  ...
  public:
    ...
    int maxSumDivThree8(vector<int>& nums) {
        ...
        for (auto v : nums) {
            int idx = v % 3;
            // looks beautful
            dp[st][0] = max({dp[ft][0], dp[ft][(3 - idx) % 3] > 0 ? dp[ft][(3 - idx) % 3] + v : 0, idx == 0 ? v : 0});
            dp[st][1] = max({dp[ft][1], dp[ft][(4 - idx) % 3] > 0 ? dp[ft][(4 - idx) % 3] + v : 0, idx == 1 ? v : 0});
            dp[st][2] = max({dp[ft][2], dp[ft][(2 - idx) % 3] > 0 ? dp[ft][(2 - idx) % 3] + v : 0, idx == 2 ? v : 0});
            swap(ft, st);
        }
        return dp[ft][0];
    }
    ...
    int maxSumDivThree2(vector<int>& nums) {
        ...
        for (auto v : nums) {
            ...
            if (idx == 0) {
                dp[st][0] = dp[ft][0] + v;
                dp[st][1] = dp[ft][1] > 0 ? dp[ft][1] + v : 0;
                dp[st][2] = dp[ft][2] > 0 ? dp[ft][2] + v : 0;
            } else if (idx == 1) {
                dp[st][0] = max(dp[ft][0], dp[ft][2] > 0 ? dp[ft][2] + v : 0);
                dp[st][1] = max({dp[ft][1], dp[ft][0] > 0 ? dp[ft][0] + v : 0, v});
                dp[st][2] = max(dp[ft][2], dp[ft][1] > 0 ? dp[ft][1] + v : 0);
            } else {
                dp[st][0] = max(dp[ft][0], dp[ft][1] > 0 ? dp[ft][1] + v : 0);
                dp[st][1] = max(dp[ft][1], dp[ft][2] > 0 ? dp[ft][2] + v : 0);
                dp[st][2] = max({dp[ft][2], dp[ft][0] > 0 ? dp[ft][0] + v : 0, v});
            }
            swap(ft, st);
        }
        return dp[ft][0];
    ...
    int maxSumDivThree6(vector<int>& nums) {
        ...
        auto dfs = [&](auto&& dfs, int i, int j) { 
            if (i < 0) {
                return j ? INT_MIN / 2 : 0;
            }
            if (dp[i][j] != -1) {
                return dp[i][j];
            }
            return dp[i][j] = max(dfs(dfs, i - 1, j), dfs(dfs, i - 1, (j + nums[i]) % 3) + nums[i]); 
        };
    ...
    int maxSumDivThree1(vector<int>& nums) {
        ...
        for (auto v : nums) {
            ...
            pre[idx - 1].push_back(v);
        }
        ranges::sort(pre[0], greater<>{});
        ranges::sort(pre[1], greater<>{});
        int i = 0, j = 0;
        while (i < pre[0].size() and j < pre[1].size()) {
            ans += pre[0][i] + pre[1][j];
            i++, j++;
        }
        for (; i + 3 < pre[0].size(); i += 3) {
            ans += pre[0][i] + pre[0][i + 1] + pre[0][i + 2];
        }

        for (; j + 3 < pre[1].size(); j += 3) {
            ans += pre[1][j] + pre[1][j + 1] + pre[1][j + 2];
        }
        return ans;
    }
    ...
    // 超时
int maxSumDivThree3(vector<int>& nums) {
    auto handler = [&](int i, int j) -> pair<bool, int> {
        if (i < 0) return {true, j ? INT_MIN / 2 : 0};
        return {false, 0};
    };
    int n = nums.size();
    ...
    return cache_nvec<2, int(int, int)>(memo, {n, 3}, INT_MIN / 2, handler)(n - 1, 0);
}

    // 超时
    int maxSumDivThree4(vector<int>& nums) {
        auto handler = [&](int i, int j) -> pair<bool, int> {
            if (i < 0) {
                return {true, j ? INT_MIN / 2 : 0};
            }
        ...
        return cache<int(int, int)>(memo)(n - 1, 0);
    }
};
// @lc code=end
