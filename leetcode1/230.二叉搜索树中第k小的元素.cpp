...
class KthVisitor : public BinaryTreeVisitor {
public:
    KthVisitor(Order order, void *data) : BinaryTreeVisitor(order, data) {}
...
protected:
    bool doWhenVisit() override {
        currentFrame->extra = ++pre;
        if (getTh()->k < currentFrame->extra ) {
            return true;
        }
        if (getTh()->k == currentFrame->extra) {
            getTh()->ans = currentFrame->node->val;
            return true;
        }
        return false;
    }
...
public:
    int kthSmallest(TreeNode* root, int k) {
        ...
        data.k = k;
        data.ans = -1;
        KthVisitor visitor(In_Order, &data);
